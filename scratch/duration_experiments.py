...
def simulate_sprint(picks, capital, entry_idx, duration_days):
    ...
    for p in picks:
        ...
        for d in range(1, duration_days + 1):
            ...
            if candle["low"] <= stop_p: exit_price = stop_p; break
...
def run_experiment(exp_name, duration_days):
    print(f"Running Experiment: {exp_name} ({duration_days} days)...")
    spy_all = all_data["SPY"]["candles"]
    
    # Generate 11 windows back from the end
    test_indices = []
    curr_idx = len(spy_all) - 41 # Max 8 weeks + buffer
    for _ in range(NUM_BACKTESTS):
        test_indices.append(curr_idx)
        curr_idx -= duration_days # Non-overlapping windows
    ...
    for idx in test_indices:
        ...
        capital += simulate_sprint(picks, capital, idx, duration_days)
        
    total_return = ((capital - PORTFOLIO_START) / PORTFOLIO_START) * 100
    return {"name": exp_name, "return": total_return}

if __name__ == "__main__":
    e1 = run_experiment("Control (3 Week / 15 Day)", duration_days=15)
    e2 = run_experiment("4 Week Sprint (20 Day)", duration_days=20)
    e3 = run_experiment("6 Week Sprint (30 Day)", duration_days=30)
    e4 = run_experiment("8 Week Sprint (40 Day)", duration_days=40)
