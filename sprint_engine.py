"""
Sprint Trading Engine - Performance Edition
Restores the high-performing 9%+ return logic while maintaining Unified UI.
1. Sector RRG: Filter for Leading/Improving sectors.
2. Selection: Top RS stocks in those sectors (Matches stock_rotation.html leaders).
3. OCO: 3R targets, adaptive ATR stops (2.0x ATR for breath).
4. UI: ETF tickers (XLK, XLP, etc.) and teal characteristics.
"""
...
def simulate_sprint_v3(all_data, picks, capital, entry_idx):
    ...
    for p in picks:
        ...
        # Breath: 2.0x ATR for lower stop-outs
        stop_dist = max(atr * 2.0, price * 0.02)
        ...
        for d in range(1, SPRINT_DAYS + 1):
            ...
            l, h, c = candles[idx]["low"], candles[idx]["high"], candles[idx]["close"]
        ...
        trades.append({"ticker":ticker, "pnl":(exit_price - price) * shares, "result":result})
...
def main():
    ...
    if ema21_now > sma50_now:
        ...
        target_db_sectors = {db for db, etf in DB_SECTOR_MAP.items() if etf in winning_etfs}
        ...
        for p in candidates[:NUM_PICKS]:
            ...
            current_sprint["orders"].append({
                "ticker":ticker, "name":p.get("name",""), "sector":f"{db_sector} ({DB_SECTOR_MAP.get(db_sector,'??')})",
                "highlights":p.get("highlights",""), "price":price, "buy_stop":price, "buy_limit":round(price*1.002,2),
                "stop":round(price-stop_dist,2), "stop_limit":round((price-stop_dist)*0.998,2), "target":round(price+stop_dist*TARGET_R,2),
    ...
    output = {
        ...
        "summary": {
            ...
            "next_sprint_reminder": "Cycle every 15 days. Strategy: Unified Sector Rotation Leaders."
        },
