...
*   **Data Integrity:** **NEVER** deploy local `.json` or `.csv` files to the repository. Remote data files are generated by daily automated scans on the server/GitHub Actions and must be preserved. Local copies are for testing only and should be excluded via `.gitignore` or manual verification.
...
*   **v5.5 Restoration (Current):**
    ...
    *   **Quadrant Spread**: Centered RRG medians on the **currently filtered group** (top 40) to ensure a dynamic 4-quadrant spread among leaders.
    *   **Card Unification**: Unified the `stock-card` component across Dashboard, Sector, and Stock Rotation pages for visual parity and layout consistency.
    *   **Safety Guards**: Added NaN/Empty-history guards to RRG rendering to prevent chart-breaking outliers.
